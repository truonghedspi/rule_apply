...
//		}
	...
	public void extendLattice() {
		for (Vertex vertex:getVertexs()) {
			if (vertex != getEndVertex()) {
//				Edge newEdge = new Edge(poss[i], words[i], false);
//				addEdge(newEdge, tmp, newVertex);
//				tmp = newVertex;
				
				Edge mainEdge = vertex.getEdgeOnMainPath();
				
				if (Utils.isRomajiString(mainEdge.getWord())) continue;
				if (Utils.isHiraganaString(mainEdge.getWord())) continue;
				if (Utils.isKatakanaString(mainEdge.getWord())) continue;
				
				Edge newEdge = new Edge(mainEdge.getPos(),Utils.convertKana(mainEdge.getPos()),false);
				addEdge(newEdge,vertex,mainEdge.getToVertex());
			}
		}
	}
